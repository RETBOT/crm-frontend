...
import { FiSearch, FiFilter, FiUser, FiMapPin, FiPhone, FiDollarSign, FiXCircle, FiPlus, FiChevronDown, FiChevronUp, FiEdit } from "react-icons/fi";
import { getClientes, getContactos, getSucursales, getVendedores, getPuestos, contactos_ABC, clientes_ABC, convertirProspecto } from "../../api/accounts";
...
export function Prospects() {
  ...
  const fetchProspects = async () => {
    ...
    setError(null);
    try {
      const res = await getClientes(0, filters.searchTerm, filters.sucursal, filters.status, filters.salesRep, page, 20, 'PROSPECTO');
      ...
      logger.error("Error al obtener prospectos:", e);
  ...
  const handleUpdateProspect = async (customerData) => {
    if (!editingProspect?.CLIENTEID) return;
    if (!hasPermission("prospects.update")) {
      showNotification("No cuenta con permisos para editar prospectos", "error");
      return;
    }
    ...
    try {
      const response = await clientes_ABC({
        ...customerData,
        CLIENTEID: editingProspect.CLIENTEID,
        TIPO_CLIENTE: "PROSPECTO",
        TIPO: "C",
      });
  ...
  const handleDeleteProspect = async (prospect) => {
    if (!hasPermission("prospects.delete")) {
      showNotification("No cuenta con permisos para eliminar prospectos", "error");
      return;
    }

    if (!window.confirm(`¿Desea inactivar el prospecto ${prospect.NOMBRECLI}?`)) return;

    try {
      const response = await clientes_ABC({
        CLIENTEID: prospect.CLIENTEID,
        NOMBRECLI: prospect.NOMBRECLI || "N/A",
        TIPO_CLIENTE: "PROSPECTO",
        TIPO: "B",
      });
      ...
      showNotification(response.msg || "No se pudo inactivar el prospecto", "error");
    } catch (error) {
      showNotification(error.message || "Ocurrió un error al inactivar el prospecto", "error");
    }
  };

  const handleConvertProspect = async () => {
    if (!selectedAccount?.CLIENTEID) return;
    if (!hasPermission("prospects.convert")) {
      showNotification("No cuenta con permisos para convertir prospectos", "error");
      return;
    }

    try {
      const response = await convertirProspecto(selectedAccount.CLIENTEID);
      if (response.resultado === 1) {
      ...
      showNotification(response.msg || "No se pudo convertir el prospecto", "error");
    } catch (error) {
      showNotification(error.message || "Error al convertir prospecto", "error");
    }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Prospectos</h1>
        {hasPermission("prospects.create") && (
          <button
            ...
            onClick={() => setShowProspectForm((prev) => !prev)}
          >
            <FiPlus className="mr-1" /> {showProspectForm ? "Ocultar formulario" : "Nuevo prospecto"}
          </button>
        )}
      </div>
              ...
              <div className="flex items-center gap-1">
                {hasPermission("prospects.update") && (
                  <button
                    ...
                    onClick={() => setEditingProspect(selectedAccount)}
                ...
                {hasPermission("prospects.delete") && (
                  <button
                    ...
                    onClick={() => handleDeleteProspect(selectedAccount)}
                    title="Inactivar prospecto"
                  >
                    <FiXCircle size={18} />
                  </button>
                )}
              ...
              <table className="w-full">
                <thead className="bg-gray-100">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-sm font-medium text-gray-700">Vendedor</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    prospects.map(acc => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm">{acc.VENDEDORID ? `${acc.VENDEDORID}-` : ''}{acc.VENDEDOR || ''}</td>
                      </tr>
                    ))
                  )}
                </tbody>
            ...
            <div className="lg:w-2/3 bg-white rounded-lg shadow overflow-hidden">
              <div className="p-4 border-b flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  {hasPermission("prospects.convert") && (
                    <button
